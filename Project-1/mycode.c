...
int main(){
	...
	shape = rectangular; // Pick one of them
...
void ShowPokemon(int shape, double body_size, int color, double move_length, int attack_power){
	...
	if(shape==1){ //square
		...
		double one_edge_sqr;
		one_edge_sqr=sqrt(body_size);
		for (i = 0; i < one_edge_sqr; ++i)//for row
		{
			for (j = 0; j < one_edge_sqr; ++j)// for column 
			{
				printf("X");
			}
			printf("\n");
		...
		for (i = 0; i < 5; ++i)//for row(5)
		{
			for (j = 0; j < long_edge_rect; ++j)// for column 
			{
				printf("X");
			}
			printf("\n");
